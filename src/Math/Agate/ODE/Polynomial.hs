...
instance (Enum v, Bounded v, Ord v, Show v, Eq k, Num k) => ODESystem (PolynomialODE k v) where
    ...
    var = Poly.var
    eval assignment expr = Poly.eval expr $ enumerate <&> (\v -> (Poly.var v, assignment v))
